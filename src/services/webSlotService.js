...
function cleanUserId(value) { const id = Number(value); return Number.isFinite(id) && id > 0 ? Math.floor(id) : null; }
function parseBet(value) { const n = Number(String(value || '').replace(/,/g, '').trim()); return Number.isFinite(n) ? Math.floor(n) : NaN; }
function checkCooldown(userId) { const waitMs = Math.max(0, Number(SLOT.cooldownMs || 700)); const now = Date.now(); const until = cooldowns.get(userId) || 0; if (until > now) return Math.ceil((until - now) / 1000); cooldowns.set(userId, now + waitMs); return 0; }
