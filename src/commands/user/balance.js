...
function payKeyboard(ctx){const username=String(getBotInfo()?.username||process.env.BOT_USERNAME||'').replace(/^@/,'');if(!username)return undefined;const url=`https://t.me/${username}?startapp=pay`;return {inline_keyboard:[[{text:'💸 Pay Bal',...(ctx.chat?.type==='private'?{web_app:{url:`${String(process.env.MINIAPP_URL||process.env.WEB_APP_URL||'').replace(/\/+$/,'')||url}`}:{url})}]]};}
